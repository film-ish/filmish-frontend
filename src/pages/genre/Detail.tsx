...
const Detail = () => {
  ...
  const { moviesByGenre, getMoviesByGenre: getRecommendedMoviesByGenre } = useRecommendStore();
    ...
    return (
      <div className="flex justify-center items-center py-6 gap-2">
        ...
        {pageNumbers.map((page, index) =>
          page === '...' ? (
            <span key={`ellipsis-${index}`} className="text-sm text-gray-400 mx-1">
              ...
            </span>
          ) : (
            <Button
  ...
  if (!genres || genres.length === 0) {
    return (
      <div className="flex justify-center items-center h-screen bg-black text-white">장르 정보를 불러오는 중...</div>
    );
  }

  if (!currentGenre) {
    return (
      <div className="flex justify-center items-center h-screen bg-black text-white">해당 장르를 찾을 수 없습니다.</div>
    );
  }
    ...
    <div className="text-white min-h-screen pb-16">
      ...
      <div className="px-4 mt-6 mb-8">
        <div className="flex justify-between items-center mb-2 mx-10">
          ...
          <Link
            to={`${ROUTES.GENRE.ROOT}/${currentGenre.id}/${ROUTES.GENRE.RECOMMENDATIONS}`}
        ...
        {recommendedMovies.length > 0 ? (
          ...
          <div className="flex justify-center items-center h-40 text-gray-400">추천 영화가 없습니다.</div>
        )}
      </div>
      ...
      <div className="px-4 mb-8 mt-15 pt-20" ref={genreSectionRef}>
        ...
        {/* 로딩 및 에러 상태 처리 */}
        {isMoviesLoading ? (
          <div className="flex justify-center items-center h-64 text-white">영화 목록을 불러오는 중...</div>
        ) : moviesError ? (
          <div className="flex flex-col justify-center items-center h-64 text-white">
            <p className="mb-4">영화 목록을 불러오는데 실패했습니다.</p>
            <Button variant="filled" shape="rounded" size="medium" onClick={() => fetchGenreMovies(currentPage)}>
              다시 시도
            </Button>
          </div>
        ) : (
              ...
              {allGenreMovies.map((movieData) => {
                ...
                return (
                  <div
                    ...
                    <MovieCard width="100%" movie={movie} isLoggedIn={false} iconType="star" />
                  </div>
                );
              })}
            </div>
