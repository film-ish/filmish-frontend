...
const Recommendations = () => {
  ...
  const { genres, getMoviesByGenre, likedMovies: storeLikedMovies, toggleLikeMovie } = useRecommendStore();
  ...
  // 현재 페이지의 영화만 표시
  const currentPageMovies = recommendedMovies.slice(currentPage * pageSize, (currentPage + 1) * pageSize);
  ...
  return (
    <div className="h-screen text-white min-h-screen pb-16">
      {/* 헤더 */}
      ...
      <div className="px-4 mb-8 mt-6">
        ...
        {/* 로딩 상태 표시 */}
        {isLoading ? (
          <div className="flex justify-center items-center h-64 text-white">추천 영화를 불러오는 중...</div>
        ) : error ? (
          <div className="flex flex-col justify-center items-center h-64 text-white">
            <p className="mb-4">추천 영화를 불러오는데 실패했습니다.</p>
            <Button variant="filled" shape="rounded" size="medium" onClick={() => loadRecommendedMovies()}>
              다시 시도
            </Button>
          </div>
        ) : (
