...
const Main = () => {
    ...
    const [scrollStates, setScrollStates] = useState<Record<string, {
    ...
    // 캐러셀 스크롤 핸들러
    const handleCarouselScroll = useCallback((ratingId: string, direction: 'prev' | 'next') => {
        // 자동 스크롤 차단 설정
...
// 필터링된 평점 범위만 표시
    ...
    // 로딩 상태 표시
    if (isLoading) {
        return <div className="flex justify-center items-center h-screen text-white">평점별 영화 목록을 불러오는 중...</div>;
    }
                ...
                {moviesByRating.map((ratingData) => {
                    ...
                    const movies = ratingData.movies.map(movie => {
                        ...
                        return {
                            ...
                            image: movie.posterUrl !== 'default_poster.jpg'
                                ? movie.posterUrl
